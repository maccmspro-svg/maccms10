...
class Provide extends Base
{
    ...
    public function vod()
    {
        ...
        if(empty($html) || $cache_time==0) {
            ...
            // 增加年份筛选 https://github.com/maccmspro-svg/maccms10/issues/815
            ...
            // 采集播放组支持多个播放器
            // https://github.com/maccmspro-svg/maccms10/issues/888
        ...
        // https://github.com/maccmspro-svg/maccms10/issues/818 影片的播放量+1
        if (
    ...
    public function vod_json($res)
    {
        ...
        foreach($res['list'] as $k=>&$v){
            ...
            if ($this->_param['ac']=='videolist' || $this->_param['ac']=='detail') {
                ...
                // https://github.com/maccmspro-svg/maccms10/issues/957
                if (!empty($GLOBALS['config']['api']['vod']['from'])) {
                    // 准备数据，逐个处理
