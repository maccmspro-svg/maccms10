...
class Type extends Base {
    ...
    public function listCacheData($lp)
    {
        ...
        $midN = (isset($mid) && $mid !== '' && is_numeric($mid)) ? intval($mid) : 0;
        if ($midN > 0 && in_array($midN, [1, 2, 8, 11, 12, 13], true)) {
            $where['type_mid'] = ['eq', $midN];
        }
        if(!empty($flag)){
            if($flag=='vod'){
                $where['type_mid'] = ['eq',1];
            ...
            elseif($flag=='manga'){
                $where['type_mid'] = ['eq',12];
            }
            elseif($flag=='goods'){
                $where['type_mid'] = ['eq',13];
            }
        }
    ...
    public function delData($where)
    {
        ...
        foreach($list as $k=>$v){
            ...
            $mid = intval($v['type_mid']);
            if (!isset($modelMap[$mid])) {
                continue;
            }
            $cc = model($modelMap[$mid])->where($where2)->count();
            if($cc > 0){
    ...
    public function moveData($where,$val)
    {
        ...
        foreach($list as $k=>$v){
            ...
            $mid = intval($v['type_mid']);
            if (!isset($modelMap[$mid])) {
                return ['code'=>1012,'msg'=>lang('model/type/move_err').'：'. $v['type_name']];
            }
            $cc = model($modelMap[$mid])->where($where2)->update($update);
            if($cc ===false){
