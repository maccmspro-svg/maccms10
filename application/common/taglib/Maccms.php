...
class Maccms extends Taglib {
	...
	protected $tags = [
        ...
        // 商品标签：参数风格尽量与 vod/art 保持一致，便于前台复用
        'goods' => ['attr' =>'order,by,start,num,id,ids,not,paging,pageurl,type,typenot,class,tag,level,letter,half,rel,timeadd,timehits,time,hitsmonth,hitsweek,hitsday,hits,cachetime'],
    ...
    public function tagForeach($tag,$content)
    {
        ...
        // foreach标签强化
        // https://github.com/maccmspro-svg/maccms10/issues/984
        $parse_addon = '';
        if(!empty($tag['offset'])){
    ...
    public function tagGoods($tag,$content)
    {
        ...
        $parse = '<?php ';
        $parse .= '$__TAG__ = \'' . json_encode($tag) . '\';';
        $parse .= '$__LIST__ = model("Goods")->listCacheData($__TAG__);';
        if($tag['paging']=='yes'){
            $parse .= '$__PAGING__ = mac_page_param($__LIST__[\'total\'],$__LIST__[\'limit\'],$__LIST__[\'page\'],$__LIST__[\'pageurl\'],$__LIST__[\'half\']);';
        }
        $parse .= ' ?>';
