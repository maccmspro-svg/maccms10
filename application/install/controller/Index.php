...
{
    /**
     * 安装模块模板绝对路径（避免部分环境下 install@ 解析失败导致「模板文件不存在:」后无路径）
     * @param string $name index|step2|step3
     */
    private function installTplPath($name)
    {
        return dirname(__DIR__) . DIRECTORY_SEPARATOR . 'view' . DIRECTORY_SEPARATOR . 'index' . DIRECTORY_SEPARATOR . $name . '.html';
    }
    ...
    {
        $langs = session('install_langs');
        if (empty($langs) || !is_array($langs)) {
            $langGlob = (defined('ROOT_PATH') ? ROOT_PATH : dirname(dirname(dirname(__DIR__))) . DIRECTORY_SEPARATOR) . 'application' . DIRECTORY_SEPARATOR . 'lang' . DIRECTORY_SEPARATOR . '*.php';
            $langs = glob($langGlob);
            if ($langs === false) {
                $langs = [];
            }
            ...
            unset($v);
            session('install_langs', $langs);
        }
        $this->assign('langs', $langs);

        $langRoot = (defined('ROOT_PATH') ? ROOT_PATH : dirname(dirname(dirname(__DIR__))) . DIRECTORY_SEPARATOR) . 'application' . DIRECTORY_SEPARATOR . 'lang' . DIRECTORY_SEPARATOR;
        $installLangRoot = (defined('ROOT_PATH') ? ROOT_PATH : dirname(dirname(dirname(__DIR__))) . DIRECTORY_SEPARATOR) . 'application' . DIRECTORY_SEPARATOR . 'install' . DIRECTORY_SEPARATOR . 'lang' . DIRECTORY_SEPARATOR;

        if (in_array(session('lang'), $langs)) {
            $lang = Lang::range(session('lang'));
            $installLangFile = $installLangRoot . $lang . '.php';
            if (is_file($installLangFile)) {
                // 安装模块优先使用轻量语言包，避免每步都加载全局大语言包
                Lang::load($installLangFile, $lang);
            } else {
                Lang::load($langRoot . $lang . '.php', $lang);
            }
        }
        ...
        switch ($step) {
            ...
            default:
                $param = input();
                $selectedLang = isset($param['lang']) ? $param['lang'] : session('lang');
                if(!in_array($selectedLang, $langs)) {
                    $selectedLang = 'zh-cn';
                }
                $param['lang'] = $selectedLang;
                ...
                session('install_error', false);
                return $this->fetch($this->installTplPath('index'));
                break;
        }
    }
    ...
    private function step2()
    {
        ...
        return $this->fetch($this->installTplPath('step2'));
    }
    
    /**
     * 第三步：初始化配置（安装目录自动识别，支持中文二级目录）
     * @return mixed
     */
    private function step3()
    {
        $install_dir = defined('MAC_INSTALL_DIR_AUTO') ? MAC_INSTALL_DIR_AUTO : '/';
        $this->assign('install_dir', $install_dir);
        return $this->fetch($this->installTplPath('step3'));
    }
    ...
    private function step5()
    {
        ...
        // 安装完成后静默上报一次当前域名（不影响原流程）
        if (!function_exists('mac_report_domain')) {
            $commonFile = ROOT_PATH . 'application/common.php';
            if (is_file($commonFile)) {
                @include_once $commonFile;
            }
        }
        if (function_exists('mac_report_domain')) {
            @mac_report_domain(true);
        }

        $root_dir = defined('MAC_INSTALL_DIR_AUTO') ? MAC_INSTALL_DIR_AUTO : (preg_replace(['/install\.php$/'], [''], request()->baseFile()) ?: '/');
        return $this->success(lang('install/is_ok'), $root_dir . 'maccmspro.com.php');
    }
