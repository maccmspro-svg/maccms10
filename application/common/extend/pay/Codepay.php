...
class Codepay {
    ...
    public function notify()
    {
        ...
        if (!$param['pay_no'] || md5(substr($sign,0,-1).trim( $GLOBALS['config']['pay']['codepay']['appkey'])) != $param['sign']) {
            ...
            $orderCode = $param['pay_id'];
            $tradeNo = isset($param['pay_no']) ? $param['pay_no'] : '';
            $amount = isset($param['money']) ? floatval($param['money']) : 0;
            if (strpos($orderCode, 'GDS') === 0) {
                $res = model('GoodsOrder')->notify($orderCode, 'codepay', $tradeNo, $amount, $param);
            } else {
                $res = model('Order')->notify($orderCode, 'codepay');
            }
            if($res['code'] >1){
                echo 'fail2';
            }
