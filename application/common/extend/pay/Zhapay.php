...
class Zhapay {
    ...
    public function notify()
    {
        ...
        if (!$param['transaction_id'] || md5($sign. $GLOBALS['config']['pay']['zhapay']['appkey']) != $param['sign']) {
            ...
            $tradeNo = isset($param['transaction_id']) ? $param['transaction_id'] : '';
            $amount = isset($param['total_fee']) ? floatval($param['total_fee']) : 0;
            if (strpos($orderCode, 'GDS') === 0) {
                $res = model('GoodsOrder')->notify($orderCode, 'zhapay', $tradeNo, $amount, $param);
            } else {
                $res = model('Order')->notify($orderCode, 'zhapay');
            }
            if($res['code'] >1){
                echo 'fail2';
            }
