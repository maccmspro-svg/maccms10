    ...
    public function listCacheData($lp,$field='*')
    {
        ...
        if($paging=='yes') {
            ...
            if($pageurl=='vod/type' || $pageurl=='vod/show'){
                ...
                if ($type < 1 && !empty($param['id'])) {
                    $type = intval($param['id']);
                }
                $type_list = model('Type')->getCache('type_list');
                if (!isset($type_list[$type])) {
                    $tmp_keys = array_keys((array)$type_list);
                    $type = !empty($tmp_keys) ? intval($tmp_keys[0]) : 0;
                }
                $type_info = isset($type_list[$type]) ? $type_list[$type] : [];
                $flag='type';
                if($pageurl == 'vod/show'){
                    $flag='show';
                }
        ...
        // 优化随机视频排序rnd的性能问题
        // https://github.com/maccmspro-svg/maccms10/issues/967
        $use_rand = false;
        if($by=='rnd'){
            $use_rand = true;
