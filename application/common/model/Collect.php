...
class Collect extends Base {
    ...
    public function vod_data($param,$data,$show=1)
    {
        ...
        foreach($data['data'] as $k=>$v){
            ...
            if ($v['type_id'] ==0) {
                ...
                // 使用资源站的添加时间，更新时间保持当前
                // https://github.com/maccmspro-svg/maccms10/issues/780
                if (empty($v['vod_time_add']) || strlen($v['vod_time_add']) != 10) {
                    $v['vod_time_add'] = time();
                }
                // 支持外部自定义修改时间
                // https://github.com/maccmspro-svg/maccms10/issues/862
                $v['vod_time'] = time();
                ...
                if (!$info) {
                    ...
                    elseif($param['opt'] == 1){
                            ...
                            foreach ($cj_down_from_arr as $k2 => $v2) {
                                ...
                                if ($cj_down_url == $info['vod_down_url']) {
                                    ...
                                    if ($arr1[$down_key] == $cj_down_url) {
                                        ...
                                        // https://github.com/maccmspro-svg/maccms10/issues/893
                                        if ($config['urlrole'] == 1) {
                        ...
                        if (strpos(',' . $config['uprule'], 'c')!==false && !empty($v['vod_serial']) && $v['vod_serial']!=$info['vod_serial']) {
                            ...
                            // 连载数如果均为整数，则取较大值
                            // https://github.com/maccmspro-svg/maccms10/issues/878
    ...
    public function website_data($param,$data,$show=1)
    {
        ...
        foreach($data['data'] as $k=>$v){
            ...
            elseif( mac_array_filter($filter_arr,$v['website_name'])!==false) {
                ...
                // 采集网址入库重复规则建议增加跳转url
                // https://github.com/maccmspro-svg/maccms10/issues/1071
                if (strpos($config['inrule'], 'c')!==false) {
    ...
    /**
     * 检查url合法性
     * https://github.com/maccmspro-svg/maccms10/issues/763
     */
