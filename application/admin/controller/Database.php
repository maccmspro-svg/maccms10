...
class Database extends Base
{
    ...
    public function sql()
    {
        if($this->request->isPost()){
            ...
            if (!empty($sql)) {
                // 只允许安全的只读语句，防止任意写入/删库
                $normalized = ltrim(strtolower($sql));
                // 禁止多语句执行
                if (strpos($normalized, ';') !== false) {
                    return $this->error(lang('format_err'));
                }
                // 仅允许以 select/show/explain 开头的查询
                if (!preg_match('/^(select|show|explain)\b/i', $normalized)) {
                    return $this->error(lang('format_err'));
                }
                $forbidden_keywords = ['into dumpfile', 'into outfile', 'load_file', 'union select', 'information_schema'];
    ...
    public function rep()
    {
        if($this->request->isPost()){
            $param = input();
            $table   = isset($param['table']) ? trim($param['table']) : '';
            $field   = isset($param['field']) ? trim($param['field']) : '';
            $findstr = isset($param['findstr']) ? $param['findstr'] : '';
            $tostr   = isset($param['tostr']) ? $param['tostr'] : '';
            $where   = isset($param['where']) ? trim($param['where']) : '';
            ...
            if (!empty($table) && !empty($field) && !empty($findstr) && !empty($tostr)) {
                // 字段名必须是真实存在的字段，防止任意拼接
                $columns = Db::getTableFields($table);
                if (!in_array($field, $columns, true)) {
                    return $this->error(lang('param_err'));
                }

                // 对 where 做简单白名单限制，只允许 and/空格/字母数字下划线和常见比较符
                if (!empty($where) && !preg_match('/^\s*(and\s+)?[a-zA-Z0-9_]+\s*(=|>|<|>=|<=|like)\s*.+$/i', $where)) {
                    return $this->error(lang('param_err'));
                }

                // 安全转义查找/替换内容
                $findEscaped = addslashes($findstr);
                $toEscaped   = addslashes($tostr);
                $sql = "UPDATE {$table} SET {$field}=REPLACE({$field},'{$findEscaped}','{$toEscaped}') WHERE 1=1 {$where}";
                Db::execute($sql);
