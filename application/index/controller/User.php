...
class User extends Base
{
    ...
    public function ajax_ulog()
    {
        ...
        if ($param['ac'] == 'set') {
            ...
            if ($data['ulog_mid'] == 13 && $data['ulog_type'] == 2) {
                $gres = model('Goods')->infoData(['goods_id' => ['eq', $data['ulog_rid']]]);
                if ($gres['code'] > 1) {
                    return json($gres);
                }
            }
    ...
    public function login()
    {
        if (Request()->isPost()) {
            ...
            return $this->success($res['msg'], url('user/index'));
        }
    ...
    public function portrait()
    {
        if(request()->isPost()){
            if ($GLOBALS['config']['user']['portrait_status'] == 0) {
                $res = ['code' => 0, 'msg' => lang('index/portrait_tip1')];
                return request()->isAjax() ? json($res) : $this->error($res['msg']);
            }
            $param=[];
            $param['input'] = 'file';
            $param['flag'] = 'user';
            ...
            // 普通表单提交：使用跳转+提示，并刷新头像 cookie
            if ($res['code'] > 0) {
                if (!empty($res['file'])) {
                    cookie('user_portrait', $res['file'], ['expire' => 2592000]);
                }
                return $this->success($res['msg'], url('user/portrait'));
            }
            return $this->error($res['msg']);
        }
    ...
    public function pay()
    {
        ...
        // 仅前台隐藏未配置的支付；不得改 mac_extends_list 本身，否则后台「在线支付配置」各 Tab 会一并消失
        $extListFiltered = [];
        foreach ($extends['ext_list'] as $cl => $name) {
            if (mac_pay_gateway_configured($cl)) {
                $extListFiltered[$cl] = $name;
            }
        }
        $extends['ext_list'] = $extListFiltered;
        if (empty($extends['ext_list'])) {
            return $this->error('暂无可用支付方式，请先在后台「系统 - 在线支付配置」中填写至少一种支付接口', mac_url('user/buy'));
        }
    ...
    public function favs()
    {
        ...
        $where = [];
        $where['user_id'] = $GLOBALS['user']['user_id'];
        if(in_array($param['mid'],['1','2','3','8','13'])){
    ...
    public function goods_orders()
    {
        ...
        $where = [];
        $where['go.user_id'] = $GLOBALS['user']['user_id'];

        $order = 'go.go_id desc';
        $res = model('GoodsOrder')->listData($where, $order, $param['page'], $param['limit']);

        $pages = mac_page_param($res['total'], $param['limit'], $param['page'], url('user/goods_orders', ['page' => 'PAGELINK']));
        $this->assign('__PAGING__', $pages);
        $this->assign('param', $param);
        $this->assign('list', $res['list']);
        return $this->fetch('user/goods_orders');
    }
    ...
    public function user_address()
    {
        if (request()->isPost()) {
            ...
            $data = [];
            $data['user_id'] = $GLOBALS['user']['user_id'];
            $data['ua_name'] = isset($post['ua_name']) ? trim($post['ua_name']) : '';
            $data['ua_phone'] = isset($post['ua_phone']) ? trim($post['ua_phone']) : '';
            $data['ua_region'] = isset($post['ua_region']) ? trim($post['ua_region']) : '';
            $data['ua_address'] = isset($post['ua_address']) ? trim($post['ua_address']) : '';
            $data['ua_is_default'] = !empty($post['ua_is_default']) ? 1 : 0;
            if (!empty($post['ua_id'])) {
                $data['ua_id'] = intval($post['ua_id']);
            }
            if ($data['ua_name'] === '' || $data['ua_phone'] === '' || $data['ua_address'] === '') {
                return $this->error('请填写收货人、手机与详细地址');
            }
            $res = model('UserAddress')->saveData($data);
            if ($res['code'] > 1) {
                return $this->error($res['msg']);
            }
            return $this->success($res['msg'], url('user/user_address'));
        }

        $param = input();
        $param['page'] = intval($param['page']) < 1 ? 1 : intval($param['page']);
        $where = ['user_id' => ['eq', $GLOBALS['user']['user_id']]];
        $res = model('UserAddress')->listData($where, 'ua_is_default desc,ua_id desc', $param['page'], 20);
        $pages = mac_page_param($res['total'], 20, $param['page'], url('user/user_address', ['page' => 'PAGELINK']));
        $this->assign('__PAGING__', $pages);
        $this->assign('list', $res['list']);
        $this->assign('param', $param);

        $edit = [];
        $eid = intval(input('edit'));
        if ($eid > 0) {
            $er = model('UserAddress')->infoData([
                'ua_id'   => ['eq', $eid],
                'user_id' => ['eq', $GLOBALS['user']['user_id']],
            ]);
            if ($er['code'] == 1) {
                $edit = $er['info'];
            }
