      ...
      if ($(".ewave-xunlei-item").length) {
        var thunderLocal = {
          newTask: function (opts) {
            if (!opts || !opts.tasks) return;
            for (var i = 0; i < opts.tasks.length; i++) {
              var u = opts.tasks[i].url;
              if (u) window.open(u, '_blank');
            }
          }
        };
        $(".ewave-downlist-btns .ewave-downlist-download").click(function (e) {
          e.preventDefault();
          thunderLocal.newTask({
            tasks: [{ url: $(this).attr("data-url") }]
          });
        });
        $(".ewave-downlist-tools .ewave-downlist-download").click(function (e) {
          e.preventDefault();
          if (MAC.CheckBox.Count($(this).attr("data-target")) == 0) {
            eWave.Layer.Error("请至少选择一个数据");
            return false;
          ...
          $(".ewave-downlist-checkbox[name='" + $(this).attr("data-target") + "']").each(function () {
            if (this.checked) {
              tasks.push({ url: this.value });
            }
          });
          thunderLocal.newTask({ tasks: tasks });
        });
  ...
  'CountDown': {
    ...
    'Jump': function (obj) {
      ...
      eWave.CountDown.Run($that, countdown, function () {
        ...
        $that.text(countdown--);
        /* 已禁用倒计时结束后的自动跳转，避免异常外链；请用户手动点击页面上的跳转链接 */
      }, function () {
        if ($that.attr("data-time")) {
          $that.attr("data-time", countdown--);
        }
