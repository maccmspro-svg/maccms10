...
class Weixin {
    ...
    public function submit($user,$order,$param)
    {
        ...
        // 前台提示与其它支付方式一致；详情写日志便于排查
        if (is_array($res)) {
            @error_log('[Weixin unifiedorder] ' . json_encode($res, JSON_UNESCAPED_UNICODE));
        }
        return false;
    }
    ...
    public function notify()
    {
        ...
        if ( ($sign===$data_sign) && ($data['return_code']=='SUCCESS') && ($data['result_code']=='SUCCESS') ) {
            ...
            $tradeNo = isset($data['transaction_id']) ? $data['transaction_id'] : '';
            $amount = isset($data['total_fee']) ? floatval($data['total_fee']) / 100 : 0;
            if (strpos($orderCode, 'GDS') === 0) {
                $res = model('GoodsOrder')->notify($orderCode, 'weixin', $tradeNo, $amount, $data);
            } else {
                $res = model('Order')->notify($orderCode, 'weixin');
            }
