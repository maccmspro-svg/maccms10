...
class Ulog extends Base {
    ...
    public function listData($where,$order,$page=1,$limit=20,$start=0)
    {
        ...
        foreach($list as $k=>&$v){
            ...
            elseif($v['ulog_mid']==13){
                $goods_info = model('Goods')->infoData(['goods_id'=>['eq',$v['ulog_rid']]],'*',1);
                if($goods_info['code']==1){
                    $g = $goods_info['info'];
                    $g['link'] = mac_url('goods/detail',['id'=>$g['goods_id']]);
                    $v['data'] = [
                        'id'=>$g['goods_id'],
                        'name'=>$g['goods_name'],
                        'pic'=>mac_url_img($g['goods_pic']),
                        'link'=>$g['link'],
                        'type_name'=>'商城商品',
                        'type'=>[
                            'type_id'=>isset($g['type_id']) ? $g['type_id'] : 0,
                            'type_name'=>'商城',
                            'link'=>mac_url('goods/index'),
                        ],
                    ];
                }
                else{
    ...
    public function saveData($data)
    {
        ...
        if($data['user_id']==0 || !in_array($data['ulog_mid'],['1','2','3','8','13']) || !in_array($data['ulog_type'],['1','2','3','4','5']) ) {
