...
var MAC = {
    ...
    'Shorten': {
        ...
        'Get': function (url, call) {
            // 保持接口存在但不再请求外部服务器，直接回调原始地址
            url = url || location.href;
            if (typeof call === 'function') {
                call({ code: 0, msg: 'shorten disabled', data: { url_short: url } });
            }
            if ($('.mac_shorten').length > 0) {
                $('.mac_shorten').val(url);
    ...
    'Gbook': {
        ...
        'Submit': function () {
            if ($(".gbook_content").val() == '') {
                MAC.Pop.Msg(140, 20, '请输入您的留言!', 1000);
                return false;
            }
            MAC.Ajax(maccms.path + '/index.php/gbook/saveData', 'post', 'json', $('.gbook_form').serialize(), function (r) {
                MAC.Pop.Msg(140, 20, r.msg, 1000);
                if (r.code == 1) {
                    location.reload();
                }
                else {
        ...
        'Init': function ($obj, $mid, $jumpurl) {
            try {
                $($obj).autocomplete(maccms.path + '/index.php/ajax/suggest?mid=' + $mid, {
                    ...
                    width: 430, scrollHeight: 439, minChars: 1, matchSubset: 0,
    ...
    'User': {
        ...
        'Init': function () {
            ...
            if (MAC.Cookie.Get('user_id') != undefined && MAC.Cookie.Get('user_id') != '') {
                ...
                if ($('.mac_user').length > 0) {
                    var Portrait = MAC.User.Portrait ? MAC.User.Portrait : `${maccms.path}/static_new/images/home/user_default.png`
                    var html = `
                    <div class="mac_drop_box mac_user_box" style="display: none;">
                        <ul class="logged">
                            <li  class="user-item">
                                <div class="portrait-img">
                                    <img src="${Portrait}" class="mac_portrait" />
                                </div>
                                <span>${MAC.User.UserName}</span>
                            </li>
                            <li  class="user-item">
                                <img src="${maccms.path}/static_new/images/home/user.png" />
                                <a target="_blank" href="${url}">用户中心</a>
                            </li>
                            <li class="user-item">
                                <img src="${maccms.path}/static_new/images/home/quit.png" class="mac_portrait" />
                                <a class="logoutbt" href="javascript:;" onclick="MAC.User.Logout();" target="_self">
                                    退出
                                </a>
                            </li>
                        </ul>
                    ...
                    $('.mac_user_box').css({ 'left': position.left - 150, 'top': (position.top + h) });
                }
                ...
                var html = `
                    <div class="mac_drop_box mac_user_box" style="display: none;">
                        <ul class="logged">
                            <li class="login-item" onclick="MAC.User.Login()">
                                <div class="login-img">
                                    <img src="${maccms.path}/static_new/images/home/user_default.png" class="mac_portrait" />
                                </div>
                                <span>登录/注册</span>
