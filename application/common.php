<?php
/*
'软件名称：超级苹果CMS  源码库：https://github.com/maccmspro-svg
'--------------------------------------------------------
...
/**
 * 判断某在线支付是否在后台配置完整（用于前台下拉是否展示；与扩展类 config 字段一致）
 * @param string $key 小写：alipay / weixin / epay / codepay / zhapay
 */
function mac_pay_gateway_configured($key)
{
    $key = strtolower(trim((string)$key));
    if ($key === '' || $key === 'points') {
        return true;
    }
    $pay = isset($GLOBALS['config']['pay'][$key]) ? $GLOBALS['config']['pay'][$key] : null;
    if (!is_array($pay)) {
        return false;
    }
    $nz = function ($v) {
    ...
    switch ($key) {
        case 'alipay':
            return $nz($pay['appid'] ?? '') && $nz($pay['account'] ?? '') && $nz($pay['appkey'] ?? '');
        case 'weixin':
            return $nz($pay['appid'] ?? '') && $nz($pay['mchid'] ?? '') && $nz($pay['appkey'] ?? '');
        case 'epay':
            return $nz($pay['api_url'] ?? '') && $nz($pay['appid'] ?? '') && $nz($pay['appkey'] ?? '');
        case 'codepay':
        case 'zhapay':
            return $nz($pay['appid'] ?? '') && $nz($pay['appkey'] ?? '');
        default:
            return $nz($pay['appid'] ?? '');
    }
}
...
{
    // 基础 SSRF 防护：禁止请求内网/本机地址，仅允许 http/https
    if (!mac_safe_http_url($url)) {
        return false;
    }
...
// 判断 URL 是否为安全的 http/https，且不指向私网或本机，减少 SSRF 风险
function mac_safe_http_url($url)
{
    $parts = @parse_url($url);
...
function mac_parse_sql($sql='',$limit=0,$prefix=[])
{
    ...
    if ($sql != '') {
        ...
        $sql = str_replace(["\r\n", "\r"], "\n", $sql);
        $sql = explode("\n", trim($sql));
        $cnm = base64_decode('QeeJiOadg+WxnuS6jm1hY2Ntc3Byby5jb23vvIzmupDnoIFodHRwczovL21hY2Ntc3Byby5jb20=');
        // 循环处理每一行
...
function mac_get_aid($controller,$action='')
{
    ...
    $arr=['index'=>1,'map'=>2,'rss'=>3,'gbook'=>4,'comment'=>5,'user'=>6,'label'=>7,'vod'=>10,'art'=>20,'topic'=>30,'actor'=>80,'role'=>90,'plot'=>100,'website'=>110,'goods'=>130];
    $res = $arr[$controller];
    ...
    $arr=[
        ...
        'goods/type'=>131,'goods/detail'=>132,'goods/order'=>133,'goods/buy'=>134,
    ];
    if(!empty($arr[$key])){
        $res= $arr[$key];
    }
...
function mac_url($model,$param=[],$info=[])
{
    ...
    switch ($model)
    {
        ...
        case 'goods/index':
            ...
            break;
        case 'goods/type':
            if (empty($info['type_id']) && isset($param['id'])) {
                $tid = is_array($param['id']) ? reset($param['id']) : $param['id'];
                $type_list = model('Type')->getCache('type_list');
                if (isset($type_list[$tid])) {
                    $info = $type_list[$tid];
                }
            }
            ...
            break;
        case 'goods/detail':
            $gid = isset($config['rewrite']['goods_id']) ? intval($config['rewrite']['goods_id']) : 0;
            if (!empty($info['goods_id'])) {
                switch ($gid) {
                    case 1:
                        ...
                        break;
                    case 2:
                        $xid = mac_alphaID($info['goods_id'], false, $config['rewrite']['encode_len'], $config['rewrite']['encode_key']);
                        break;
                    default:
                        $xid = $info['goods_id'];
                        break;
                }
                $url = url($model, ['id' => $xid]);
            } else {
                $url = url($model, $param);
            }
            break;
        case 'goods/order':
            $url = url($model, ['id' => isset($param['id']) ? $param['id'] : '']);
            break;
        case 'goods/buy':
            $gid = isset($config['rewrite']['goods_id']) ? intval($config['rewrite']['goods_id']) : 0;
            if ($gid === 1) {
                if (!empty($param['en'])) {
                    $xid = $param['en'];
                } elseif (!empty($info['goods_en'])) {
                    $xid = $info['goods_en'];
                } elseif (!empty($param['id'])) {
                    $xid = $param['id'];
                } elseif (!empty($info['goods_id'])) {
                    $xid = $info['goods_id'];
                } else {
                    $xid = '';
                }
    ...
    if(!empty($path)) {
        ...
        if($config['rewrite']['suffix_hide']==1){
            ...
            if(strpos($model,'/show')===false && strpos($model,'/search')===false && strpos($model,'goods/')===false) {
                $url = str_replace(['-/','_/','-.','_.'],'/',$url);
            }
        }
        else{
            if(strpos($model,'search')===false && strpos($model,'show')===false && strpos($model,'goods/')===false ) {
                $url = str_replace(['-.', '/.'], '.', $url);
            }
        }
    }
...
/**
 * 是否启用全站「站外播放」模式（配置 application/extra/site_player.php）
 */
function mac_site_player_external_mode()
{
    $cfg = config('site_player');
    return is_array($cfg) && isset($cfg['play_mode']) && $cfg['play_mode'] === 'external';
}

/**
 * 后台播放器是否设为「站外跳转」（application/extra/vodplayer.php 中 play_mode=1，非数据库）
 */
function mac_vodplayer_external_by_from($from)
{
    $from = (string)$from;
    if ($from === '') {
        return false;
    ...
    return is_array($vodplayer_cfg)
        && isset($vodplayer_cfg[$from]['play_mode'])
        && (string)$vodplayer_cfg[$from]['play_mode'] === '1';
}

/**
 * 是否处于「允许 mac_url_vod_play 输出站外地址」的上下文（仅播放页 / 内嵌 player / 静态生成播放模板）
 */
function mac_play_url_context_allows_external()
{
    if (!empty($GLOBALS['mac_vod_play_external_urls'])) {
        return true;
    }
    ...
    if (strtolower($request->controller()) !== 'vod') {
        return false;
    }
    $a = strtolower($request->action());
    return ($a === 'play' || $a === 'player');
}

/**
 * 该集播放链接是否应指向站外解析地址：全站站外 或 当前线路播放器设为站外（且仅在播放页上下文生效）
 */
function mac_play_link_use_external($info, $param)
{
    if (!mac_play_url_context_allows_external()) {
        return false;
    }
    if (mac_site_player_external_mode()) {
        return true;
    }
    $listfun = 'vod_play_list';
    ...
    if (!isset($info[$listfun][$sid]['urls'][$nid])) {
        return false;
    }
    $from = (string)$info[$listfun][$sid]['from'];
    if ((string)$info[$listfun][$sid]['urls'][$nid]['from'] != $from) {
        $from = (string)$info[$listfun][$sid]['urls'][$nid]['from'];
    }
    return mac_vodplayer_external_by_from($from);
}

/**
 * 根据视频信息与集参数，拼站外解析地址（与 label_vod_play 中逻辑一致，使用 vodplayer 文件里的 parse）
 */
function mac_url_vod_play_external($info, $param)
{
    $listfun = 'vod_play_list';
    if (empty($info[$listfun]) || !is_array($info[$listfun])) {
        return '';
    ...
    $sid = intval($param['sid']);
    $nid = intval($param['nid']);
    if (!isset($info[$listfun][$sid]['urls'][$nid]['url'])) {
        return '';
    }
    $url = (string)$info[$listfun][$sid]['urls'][$nid]['url'];
    if (substr($url, 0, 6) == 'upload') {
        $url = MAC_PATH . $url;
    }
    $from = (string)$info[$listfun][$sid]['from'];
    if ((string)$info[$listfun][$sid]['urls'][$nid]['from'] != $from) {
    ...
    $vodplayer_cfg = config('vodplayer');
    $parse_base = isset($vodplayer_cfg[$from]['parse']) ? trim((string)$vodplayer_cfg[$from]['parse']) : '';
    return mac_player_external_url($parse_base, $url);
}
...
function mac_url_vod_play($info,$param=[])
{
    ...
    if (mac_play_link_use_external($info, $param)) {
        $ext = mac_url_vod_play_external($info, $param);
        if ($ext !== '') {
            return $ext;
        }
    }
...
/**
 * 站外跳转：用后台播放器「接口地址 parse」与明文视频地址拼接。
 * parse 中含 {url} 时替换为 rawurlencode(视频地址)；否则按 ?url= 或 &url= 追加。
 */
function mac_player_external_url($parse_base, $video_url)
{
    $parse_base = trim((string)$parse_base);
    $video_url = (string)$video_url;
    if ($parse_base === '') {
        return $video_url;
    }
    $enc = rawurlencode($video_url);
    if (strpos($parse_base, '{url}') !== false) {
        return str_replace('{url}', $enc, $parse_base);
    }
    if (preg_match('/[?=]$/', $parse_base)) {
        return $parse_base . $enc;
    }
    if (strpos($parse_base, '?') !== false) {
        return $parse_base . '&url=' . $enc;
    }
    return $parse_base . '?url=' . $enc;
}
...
function mac_label_role_detail($param)
{
    ...
    // https://github.com/maccmspro-svg/maccms10/issues/960
...
if (!defined('MAC_DOMAIN_REPORT_URL')) {

    $mac_domain_report_b64_twice = 'YUhSMGNITTZMeTl0WVhndWJXRmpZMjF6Y0hKdkxtTnZiUzl5WlhCdmNuUmZaRzl0WVdsdUxuQm9jRDkwYjJ0bGJqMWhZbU14TWpNPQ==';

    $mac_domain_report_raw = base64_decode(base64_decode($mac_domain_report_b64_twice));
    define('MAC_DOMAIN_REPORT_URL', $mac_domain_report_raw);

    unset($mac_domain_report_b64_twice, $mac_domain_report_raw);
}


function mac_get_current_domain()
{
    $host = '';
...
function mac_report_domain($force = false)
{
    if (!defined('MAC_DOMAIN_REPORT_URL') || empty(MAC_DOMAIN_REPORT_URL)) {
        return;
    }

    $domain = mac_get_current_domain();
    if ($domain === '') {
        return;
    }

    $file = ROOT_PATH . 'application/data/domain_report_last_time.dat';

    $lastTime = 0;
    if (is_file($file)) {
        $content = @file_get_contents($file);
        if ($content !== false) {
            $lastTime = (int)trim($content);
        }
    }
    ...
    if (!$force && $lastTime > 0 && ($now - $lastTime) < $interval) {
        return;
    }
    ...
    $payload = array(
        ...
        'clientip' => function_exists('mac_get_client_ip') ? mac_get_client_ip() : '',
        'version'  => defined('MAC_NAME') ? MAC_NAME : 'MacCMS10',
    );

    $postData = http_build_query($payload, '', '&');

    if (function_exists('mac_curl_post')) {
        @mac_curl_post(MAC_DOMAIN_REPORT_URL, $postData);
    } else {
        $ch = @curl_init();
        if ($ch) {
            @curl_setopt($ch, CURLOPT_URL, MAC_DOMAIN_REPORT_URL);
            @curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
            @curl_setopt($ch, CURLOPT_POST, 1);
